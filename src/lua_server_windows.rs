...
//!
//! Provides a loopback TCP server on Windows that accepts Lua code and returns results.
...
pub fn socket_path() -> PathBuf {
    PathBuf::from(format!("wow-lua-{}.tcp", std::process::id()))
}
...
pub fn init() -> mpsc::Receiver<LuaCommand> {
    ...
    thread::spawn(move || run_server(tx, path));
    rx
}

fn run_server(cmd_tx: mpsc::Sender<LuaCommand>, endpoint_path: PathBuf) {
    let listener = match TcpListener::bind("127.0.0.1:0") {
        Ok(listener) => listener,
        Err(error) => {
            crate::logging::eprintln_elapsed(&format!(
                "[wow-sim] Failed to bind loopback IPC: {error}"
            ));
            return;
        }
    };
    let endpoint = match listener.local_addr() {
        ...
        Err(error) => {
            crate::logging::eprintln_elapsed(&format!(
                "[wow-sim] Failed to read loopback IPC address: {error}"
            ));
            return;
        }
    };
    if let Err(error) = std::fs::write(&endpoint_path, &endpoint) {
        crate::logging::eprintln_elapsed(&format!(
            "[wow-sim] Failed to publish loopback IPC endpoint: {error}"
        ));
        return;
    }
    crate::logging::eprintln_elapsed(&format!("[wow-sim] Listening on {endpoint}"));
    ...
    let _guard = EndpointGuard(endpoint_path);
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                if let Err(error) = handle_connection(stream, &cmd_tx) {
                    crate::logging::eprintln_elapsed(&format!(
                        "[wow-sim] Connection error: {error}"
                    ));
                }
            }
            Err(error) => {
...
pub mod client {
    ...
    fn send_request<P: AsRef<Path>>(socket: P, request: Request) -> Result<Response, String> {
        let endpoint = std::fs::read_to_string(socket.as_ref())
            .map_err(|e| format!("Read endpoint failed: {e}"))?;
        let mut stream =
            TcpStream::connect(endpoint.trim()).map_err(|e| format!("Connect failed: {e}"))?;
